...
function StarBackground({ active }: { active: boolean }) {
  ...
  return (
    <svg
        viewBox="0 0 27 26"
        aria-hidden="true"
        className="absolute left-1/2 top-1/2 h-[26px] w-[32px] -translate-x-1/2 -translate-y-1/2"
        >
      <defs>
        <radialGradient
          id={gradientId}
        ...
        >
          ...
          <stop offset="1" stopColor="#DDD2EF" />
        </radialGradient>
      </defs>
      ...
      <path
        ...
        fill={active ? "#774CBE" : `url(#${gradientId})`}
      />
    </svg>
  )
}
...
export default function CalendarDay({
  ...
  const textClassName = (() => {
    if (isWritten) {
      return isToday ? "text-white" : "text-main-500"
    }
  ...
  return (
    <button
        ...
        className={[
            ...
            selected && !isWritten
            ? "after:absolute after:bottom-0 after:h-1 after:w-1 after:rounded-full after:bg-main-500"
            : "",
        ].join(" ")}
        >
        {isWritten && <StarBackground active={isToday} />}

    <span
        className={`relative z-10 flex h-[26px] items-center justify-center text-footnote font-normal leading-none ${textClassName}`}
    >
        {day}
    </span>
</button>
